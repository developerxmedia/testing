...
class PropertyProperty(models.AbstractModel):
   ...
   # owner_id = fields.Many2one('res.partner', string="Owner")
    ...
    responsible_id = fields.Many2one('hr.employee', string="Property Manager")
    region_id = fields.Many2one('property.region', string="Region")
    asset_number = fields.Char(string="Asset Number", index=True)
    #new
    propertyname = fields.Many2one('property.land',string="Property", index=True)
    buildingareatype = fields.Many2one('buildingarea.type',string="Building/Facility Type", index=True)    
