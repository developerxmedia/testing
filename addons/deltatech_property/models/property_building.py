...
class PropertyBuilding(models.Model):
    ...
    internalarea_count = fields.Integer(compute='compute_internalcount')
    new_building_name = fields.Char('New Building Name')
    ...
    floors_ids = fields.One2many('floor.history', 'building_id', string="Features")
    ...
    def create(self, vals):
        ...
        result.new_building_name = result.name
    ...
    def compute_roomscount(self):
         for record in self:
            record.rooms_count = self.env['property.room'].search_count(
                [('building_id', '=', record.id),('floor_type', '=', 'c')]) 
    ...
    def compute_internalcount(self):
        for record in self:
            record.internalarea_count = self.env['property.room'].search_count(
            [('building_id', '=', record.id), ('floor_type', '=', 'l')]) 
   ...
   # data_pif = fields.Date(string="Data PIF")
    def show_rooms(self):
        ...
        return {
                'name': "Rooms",
                'domain': [('building_id', '=', self.id),('floor_type', '=', 'c')],
    ...
    def show_internalarea(self):
        domain = [('building_id', '=', self.id), ('floor_type', '=', 'l')]
        return {
                'name': "Internal Area",
                'domain': domain,
                'view_mode': 'kanban,tree,form',
    ...
    def open_createbuilding(self):
        view_id = self.env.ref('deltatech_property.propertyfloors_view').id
        ...
        return {
               'name': ('Create Floor'),
                ...
                'context': {
                    'default_propertys':self.propertyname.id ,
                    'default_building_id':self.id ,
                    'default_floormanager':self.responsible_id.id ,
                        },
                 }
...
class floorHistory(models.Model):
    _name = 'floor.history'
    _rec_name = 'owner_id'

    building_id = fields.Many2one('property.building', string='Building', )
    owner_id = fields.Many2one('facility.master', string="Facility Name")
