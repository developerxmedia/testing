...
class PropertyNomenclature(models.AbstractModel):
    ...
    cod = fields.Char(string='Region')
#    name = fields.Char()
#    categ = fields.Char(string="Category")
    sercategory = fields.Many2one('property.region',string='Category')
#    parent_id = fields.Char(string='Parent')
    complete_name = fields.Char('Category Name', compute='_compute_complete_name',store=True)  
    @api.depends('cod', 'sercategory.complete_name')
    def _compute_complete_name(self):
        for category in self:
            if category.sercategory:
                category.complete_name = '%s / %s' % (category.sercategory.complete_name, category.cod)
            else:
                category.complete_name = category.cod   
