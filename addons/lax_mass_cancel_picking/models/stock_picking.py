...
class stock_move(models.Model):
    ...
    def set_quant_quantity(self, stock_move, pack_operation_ids, pic_state):
        for pack_operation_id in pack_operation_ids:
            # Default move qty is planned qty
            move_qty = stock_move.product_uom_qty

            # ✅ Fix: get done qty from move lines
            done_qty = sum(line.qty_done for line in stock_move.move_line_ids)
            if done_qty:
                move_qty = done_qty
            ...
            if done_qty:
                if stock_move.sale_line_id:
                    if stock_move.sale_line_id.qty_delivered >= move_qty:
                        stock_move.sale_line_id.qty_delivered -= move_qty

                # If you also want to rollback purchase received qty:
                # if stock_move.purchase_line_id:
                #     if stock_move.purchase_line_id.qty_received >= move_qty:
                #         stock_move.purchase_line_id.qty_received -= move_qty
