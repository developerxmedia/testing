...
class asset_categorytypes(models.Model):
    _description = 'Asset cat'
    _name = 'asset.categorytypes'

    name = fields.Char('Category', required=True, translate=True)

    categorytypes = fields.Many2one('asset.category','Types',)
...
#new
    ...
    rooms = fields.Many2one('property.room',string='Room')
    #serviceprovider = fields.Many2one('service.provider',string='Service Provider')

    serviceprovider = fields.Many2one('res.partner',string='Service Provider',domain=[('serviceprovider', '=', True)])

    serviceprovidertype = fields.Char('Service Type')    
    #serviceprovidertype = fields.Many2one('service.providertype',string='Service Type')
    startdate = fields.Date(string='Service Start')
    startend = fields.Date(string='Service End')
   # equipmenttype = fields.Char(string='Asset Type')
   # equipmenttype = fields.Many2one('equiment.type',string='Asset Type')
    equipmentcat = fields.Many2one('asset.categorytypes',string='Asset Category')
    ...
    category_ids = fields.Many2one('asset.category', string='Asset Type')
    ...
    @api.constrains('startdate','startend')
    def date(self):
        if self.startdate > self.startend:
 ...
 #   def _check_date(self):
  #      if self.startdate > self.startend:
   #     raise ValidationError('End date must be greater than start date')
    ...
    @api.onchange('equipmentcat')
    def constrains_equipmentcat(self):
        return {'domain':{'category_ids':[('name', '=', self.equipmentcat.categorytypes.name)]}}	
